...
class Object:
    def __init__(self, completed, name, **options):
        ...
        self.options = {
            ...
            "source": name,
            "asm": False,
            # FSA-specific: path to a post-compile byte-patch spec (see
            # tools/patch_obj.py) applied to this object's .text section
            # before linking, for MWCC codegen "wall" residuals that no
            # C++ source phrasing can influence (see
            # reference_fsa_param_register_pairing_wall_investigation.md).
            "patch": None,
        }
...
def generate_build_ninja(config, build_config):
    ...
    n.comment("FSA-specific: post-compile object byte-patch (see Object 'patch' option)")
    patch_obj = config.tools_dir / "patch_obj.py"
    n.rule(
        name="patch_obj",
        command=f"$python {patch_obj} $in $out $patchfile",
    ...
    if build_config:
        ...
        def add_unit(build_obj, link_step):
            ...
            patch_spec = options.get("patch")
            patched_obj_path = build_src_path / f"{base_object}.patched.o"
            ...
            if src_obj_path not in source_added:
                ...
                if patch_spec:
                    n.build(
                        outputs=path(patched_obj_path),
                        rule="patch_obj",
                        inputs=path(src_obj_path),
                        variables={"patchfile": path(Path(patch_spec))},
                        implicit=path([patch_obj, Path(patch_spec)]),
                    )
                    n.newline()
            ...
            if completed:
                obj_path = patched_obj_path if patch_spec else src_obj_path
            link_step.add(obj_path)
